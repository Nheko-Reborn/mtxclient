...
synapse: ## Start a synapse instance on docker
	...
	@echo Register alice
	@docker exec synapse /bin/sh -c 'register_new_matrix_user --admin -u alice -p secret -c /data/homeserver.yaml http://localhost:8009'
	@echo Register bob
	@docker exec synapse /bin/sh -c 'register_new_matrix_user --admin -u bob -p secret -c /data/homeserver.yaml http://localhost:8009'
	@echo Register carl
	@docker exec synapse /bin/sh -c 'register_new_matrix_user --admin -u carl -p secret -c /data/homeserver.yaml http://localhost:8009'
	@echo Register presence
	@docker exec synapse /bin/sh -c 'register_new_matrix_user --admin -u presence -p secret -c /data/homeserver.yaml http://localhost:8009'
	@echo Register presencesync
	@docker exec synapse /bin/sh -c 'register_new_matrix_user --admin -u presencesync -p secret -c /data/homeserver.yaml http://localhost:8009'
